...
async def register(
    username: str = Form(..., description="The desired username for the new account.", example="newuser"),
    email: str = Form(..., description="The email address for the new account.", example="user@example.com"),
    password: str = Form(..., description="The password for the new account.", example="a_strong_password"),
    avatar: Optional[UploadFile] = File(None, description="An optional avatar image file to upload."),
...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    if is_login_blocked(form_data.username):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Account is locked due to too many failed login attempts. Please try again in 24 hours.")
    user = db.query(Users).filter(Users.username == form_data.username).first()
    if not user or not user.hashed_password or not verify_password(form_data.password, user.hashed_password):
        if user: record_failed_login_attempt(form_data.username)
...
async def forgot_password(request: ForgotPasswordRequest, db: Session = Depends(get_db)):
    if not check_forgot_password_limit(request.email):
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="You have made too many password reset requests. Please try again in an hour.")
...
async def auth_google(request: Request, db: Session = Depends(get_db)):
    ...
    def _get_or_create_google_user_and_token(info: dict, db_session: Session) -> str:
        ...
        user = db_session.query(Users).filter(Users.google_id == google_id).first()
        if user: return create_access_token(data={"sub": str(user.id)})
        ...
        new_user = Users(google_id=google_id, email=email, username=info.get('name', email), avatar=info.get('picture'), hashed_password=None)
    ...
    if not frontend_url:
        raise HTTPException(status_code=500, detail="Frontend callback URL is not configured.")
